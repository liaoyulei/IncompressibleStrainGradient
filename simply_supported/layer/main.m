...
mu = E / 2 / (1 + nu);
syms x y;
%u = [-2*sin(pi*x)^2*sin(pi*y)^2*sin(2*pi*y); (2*mu+lambda)/lambda*sin(2*pi*x)*sin(pi*y)^4];
%u = [-2*sin(pi*x)^2*sin(pi*y)^2*sin(2*pi*y); sin(2*pi*x)*sin(pi*y)^4];
