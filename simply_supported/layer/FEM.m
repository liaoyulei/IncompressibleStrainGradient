...
%a = @(ux, uy, uxx, uxy, uyy, vx, vy, vxx, vxy, vyy) 2 * mu * (ux(1) * vx(1) + uy(2) * vy(2)) + mu * (uy(1) + ux(2)) * (vy(1) + vx(2)) + 2 * mu * (iota^2 + 1 / n^2) * (uxx(1) * vxx(1) + uxy(1) * vxy(1) + uxy(2) * vxy(2) + uyy(2) * vyy(2)) + mu * (iota^2 + 1 / n^2) * ((uxy(1) + uxx(2)) * (vxy(1) + vxx(2)) + (uyy(1) + uxy(2)) * (vyy(1) + vxy(2)));
...
for k = 1: Nt
    ...
    if sum(~[v1(1), v2(1), v3(1)]) == 2
        idx = round(n*max([v1(2), v2(2), v3(2)]));
        vxx = phixx(~v1(1)/2, ~v2(1)/2, ~v3(1)/2, sgn1, sgn2, sgn3, v1(1), v2(1), v3(1), v1(2), v2(2), v3(2));
        Y1(idx) = vxx(:, 1)' * u(mesh(k, 1: 20));
        x = v1(1) * ~v1(1)/2 + v2(1) * ~v2(1)/2 + v3(1) * ~v3(1)/2;
        y = v1(2) * ~v1(1)/2 + v2(2) * ~v2(1)/2 + v3(2) * ~v3(1)/2;
        vxx = uxx(x, y);
        Y2(idx) = vxx(1);
        vxx = phixx(1/2, 1/2, 1/2, sgn1, sgn2, sgn3, v1(1), v2(1), v3(1), v1(2), v2(2), v3(2));
        Y3(idx) = vxx(:, 1)' * u(mesh(k, 1: 20));
        x = (v1(1) + v2(1) + v3(1)) / 2;
        y = (v1(2) + v2(2) + v3(2)) / 2;
        vxx = uxx(x, y);
        Y4(idx) = vxx(1);
